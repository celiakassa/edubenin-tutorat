...
final class ListeAnnonceController extends Controller
{
    /**
    ...
    public function index(Request $request): \Illuminate\Contracts\View\Factory|\Illuminate\Contracts\View\View
    {
        ...
        if ($request->has('search') && ! empty($request->search)) {
            ...
            $annonces->where(function (\Illuminate\Contracts\Database\Query\Builder $query) use ($searchTerm): void {
                $query->whereHas('subject', function (\Illuminate\Contracts\Database\Query\Builder $q) use ($searchTerm): void {
                    $q->where('nom', 'LIKE', sprintf('%%%s%%', $searchTerm));
                })
        ...
        if ($request->has('domaine') && ! empty($request->domaine)) {
            $annonces->whereHas('subject', function (\Illuminate\Contracts\Database\Query\Builder $q) use ($request): void {
                $q->where('nom', 'LIKE', sprintf('%%%s%%', $request->domaine));
            });
        }
        ...
        if ($request->has('jour') && ! empty($request->jour)) {
            $jour = $request->jour;
            $annonces->where('disponibilite', 'LIKE', sprintf('%%%s%%', $jour));
        }

        // Récupérer toutes les matières uniques pour le filtre
        $matieres = Subject::whereHas('annonces', function (\Illuminate\Contracts\Database\Query\Builder $q): void {
     ...
     */
    public function demandesListe(Request $request): \Illuminate\Contracts\View\Factory|\Illuminate\Contracts\View\View
    {
        // Requête de base
        ...
        if ($request->has('search') && ! empty($request->search)) {
            $searchTerm = $request->search;
            $demandes->where(function (\Illuminate\Contracts\Database\Query\Builder $query) use ($searchTerm): void {
                $query->whereHas('subject', function (\Illuminate\Contracts\Database\Query\Builder $q) use ($searchTerm): void {
                    $q->where('nom', 'LIKE', sprintf('%%%s%%', $searchTerm));
                })
                    ->orWhere('description', 'LIKE', sprintf('%%%s%%', $searchTerm));
        ...
        if ($request->has('domaine') && ! empty($request->domaine)) {
            $demandes->whereHas('subject', function (\Illuminate\Contracts\Database\Query\Builder $q) use ($request): void {
                $q->where('nom', 'LIKE', sprintf('%%%s%%', $request->domaine));
            });
        }
